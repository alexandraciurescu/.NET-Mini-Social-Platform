    ...
    return View();
        ...
        public ActionResult Delete(int id)
        {
            Profile profile = db.Profiles
                                   .Where(profile => profile.Id == id).First();


            if (profile.UserId == _userManager.GetUserId(User) || User.IsInRole("Admin"))
            {
                var comments = db.Comments.Where(c => c.UserId == _userManager.GetUserId(User));
                db.Comments.RemoveRange(comments);

                var posts = db.Posts.Where(p => p.UserId == _userManager.GetUserId(User));
                db.Posts.RemoveRange(posts);

                var usergroups = db.UserGroups.Where(ug => ug.UserId == _userManager.GetUserId(User) || ug.Group.UserId == _userManager.GetUserId(User));
                db.UserGroups.RemoveRange(usergroups);

                var groups = db.Groups.Where(g => g.UserId == _userManager.GetUserId(User));
                db.Groups.RemoveRange(groups);

                var requests = db.Requests.Where(r => r.UserId1 == _userManager.GetUserId(User) || r.UserId2 == _userManager.GetUserId(User));
